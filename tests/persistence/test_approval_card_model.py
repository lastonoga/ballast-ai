"""``ApprovalCard`` SQLModel — shape + Pydantic validation."""
...
def test_json_round_trip_preserves_fields() -> None:
    ...
    assert again.status == card.status
    assert again.payload == card.payload
    assert again.resolution == card.resolution
    assert again.created_at == card.created_at
    assert again.resolved_at == card.resolved_at
