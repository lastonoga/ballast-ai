...
from pydantic_ai_stateflow.cli.app_detect import (
    AppRef,
    resolve_app_ref,
)
from pydantic_ai_stateflow.cli.main import cli
...
def test_resolve_app_ref_missing_raises(monkeypatch, tmp_path) -> None:
    monkeypatch.delenv("STATEFLOW_APP", raising=False)
    monkeypatch.chdir(tmp_path)  # away from project pyproject
    with pytest.raises(Exception, match="STATEFLOW_APP"):
        resolve_app_ref()


def test_resolve_app_ref_malformed(monkeypatch) -> None:
    monkeypatch.setenv("STATEFLOW_APP", "no-colon-here")
