...
Framework code reads it via ``ballast.get_ballast()`` (lazy lookup, raises
...
def get_ballast() -> Engine:
    """Return the process-wide engine. Raises if create_app not called."""
    if _engine is None:
        raise ConfigurationError(
            "Engine not initialized — call ballast.create_app(...) first",
            hint="ballast.create_app builds the Engine; framework code reads it via get_ballast()",
        )
    return _engine


def _set_ballast(engine: Engine) -> None:
    ...
    if _engine is not None and _engine is not engine:
        raise ConfigurationError(
            ...
            hint="One Engine per process. For tests, use _reset_ballast_for_tests() between cases.",
        )
    _engine = engine


def _reset_ballast_for_tests() -> None:
    """Test-only: clear the singleton so the next create_app builds fresh."""
    global _engine
    _engine = None


__all__ = ["Engine", "_reset_ballast_for_tests", "_set_ballast", "get_ballast"]
