...
Constructed once by ``sf.create_app(thread_repo=, event_log=, event_stream=, ...)``.
Framework code reads it via ``sf.get_engine()`` (lazy lookup, raises
...
def get_engine() -> Engine:
    ...
    if _engine is None:
        raise ConfigurationError(
            "Engine not initialized — call sf.create_app(...) first",
            hint="sf.create_app builds the Engine; framework code reads it via get_engine()",
        )
    return _engine
...
def _set_engine(engine: Engine) -> None:
    ...
    if _engine is not None and _engine is not engine:
        raise ConfigurationError(
            "sf.create_app() called twice with different configs",
