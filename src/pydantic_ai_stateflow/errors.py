...
``StateflowError`` with a stable ``code`` and a class-level
...
class ConfigurationError(StateflowError):
    code = "STATEFLOW_CONFIG"


class SettingsValidationError(ConfigurationError):
    code = "STATEFLOW_CONFIG_SETTINGS_INVALID"


class MissingDependencyError(ConfigurationError):
    code = "STATEFLOW_CONFIG_DEPENDENCY_MISSING"
...
class ConfigurationInvariantViolation(ConfigurationError):
    ...
    code = "STATEFLOW_CONFIG_INVARIANT"


# ---------- Persistence domain ----------


class PersistenceError(StateflowError):
    code = "STATEFLOW_PERSISTENCE"
...
class ThreadMetadataInvalid(PersistenceError):
    code = "STATEFLOW_PERSISTENCE_THREAD_METADATA_INVALID"
    status_code = 422
...
class AuthError(StateflowError):
    code = "STATEFLOW_AUTH"
    status_code = 401


class AuthorizationDenied(AuthError):
    code = "STATEFLOW_AUTH_FORBIDDEN"
    status_code = 403
...
class PatternError(StateflowError):
...
def _format_rich(exc: StateflowError) -> str:
